...
- For homeowners, finding a location that fits their living habits (quiet/loud neighborhood).
...
ggplot(data = df_all_agg, mapping = aes(x = year, y = sum_count)) + geom_line() + geom_point() + ggtitle("311 Calls Count Year to Year") + labs(x = "Year", y = "Count (in thousands)") + scale_y_continuous(labels = c(200,400,600), breaks = 10^5*c(2,4,6))
```
...
ggplot(data = df_agg_group, mapping = aes(x = year, y = sum_count, color = type)) + geom_point() + geom_line() + ggtitle("311 Call Count Year to Year by Incident Type") + labs(x = "Year", y = "Count (in Thousands)", color = "Incident Type") + scale_y_continuous(labels = c(100,200,300,400), breaks = 10^5*c(1,2,3,4))

ggplot(data = df_agg_group, mapping = aes(x = year, y = mean_process, color = type)) + geom_point() + geom_line() + ggtitle("Average Processing Times Year to Year by Incident Type") + labs(x = "Year", y = "Average Processing Time (hours)", color = "Incident Type")
